...
var DeliveriesStatusController = class {
  async update(request2, response) {
    ...
    const { id } = paramsShema.parse(request2.params);
...
var DeliveryLogsConstroller = class {
  async create(request2, response) {
    ...
    const { delivery_id, description } = bodyShema.parse(request2.body);
  ...
  async show(request2, response) {
    ...
    const { delivery_id } = paramSchema.parse(request2.params);
    const delivery = await prisma.delivery.findUnique({
      where: { id: delivery_id },
      include: {
        logs: true,
...
describe("SessionsController", () => {
  ...
  it("shoul authenticate a and get acces token", async () => {
    const user = await (0, import_supertest.default)(app).post("/users").send({
      name: "Teste User",
      email: "testeuser@gmail.com",
      password: "password123"
    });
    user_id = user.body.id;
    const sessions = await (0, import_supertest.default)(app).post("/sessions").send({
      email: "testeuser@gmail.com",
      password: "password123"
    });
